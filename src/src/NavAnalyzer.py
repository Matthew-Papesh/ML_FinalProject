...
class NavAnalyzer: 
    ...
    def analyzePerformance(self, epochs: int):
        ...
        for epoch in range(0, epochs):
            ...
            x_errors, y_errors, heading_errors, lin_speeds, ang_speeds, lin_adjs, ang_adjs = self.requestNavSimTest(lin_kp=self.LIN_KP, lin_ki=self.LIN_KI, lin_kd=self.LIN_KD, ang_kp=self.ANG_KP, ang_ki=self.ANG_KI, ang_kd=self.ANG_KD)
            N = min(len(x_errors), min(len(y_errors), min(len(heading_errors), min(len(lin_speeds), min(len(ang_speeds), min(len(lin_adjs), len(ang_adjs)))))))
            # log errors
            for i in range(0, N):
                error_log.append((x_errors[i], y_errors[i], heading_errors[i], lin_speeds[i], ang_speeds[i]))
